...
if($login->is_loggedin()!="")
{
 ...
 if(isset($_POST['Submittt'])){
		...
		$stmt = $Common->runQuery("SELECT * FROM teachers WHERE mobile = '$mobile'");
		$stmt->execute();
	...
	{ 
		...
		$varificationcode  = uniqid();
	 	$stmt = $Common->runQuery("Update teachers Set `token` = '$varificationcode' Where mobile='$mobile'");
		$stmt->execute(); 
		$InsertId =$result['id'];	
		//$user = urlencode(base64_encode($InsertId));
		$Token = $varificationcode;
		$Name = $result["firstname"];
		$Email = $result["email"];
		
	 $Sevt = $SUPPORT->send_forgot_pass_user($Name,$Email,$Token);
		if($Sevt == '1')
		{	
			$message = 'An  email will be sent to your email address that includes a password reset link.';	
		}
		else
		{
			$error[] = 'Somthing Going ?To Wrong Please Try again later.';		
		}
	} 
	else
	{
		$error[] = 'This Mobile No. you entered is not valid.';	
	}
}
?>
<!DOCTYPE html>
    ...
    <div class="container">
        <div class="row">
            ...
            <?php $Common->redirectwithtime('index.php',3000); }  ?> 
                    <span class="heading">What's My Password</span>
                    <div class="form-group">
                     <input  name="mobile" type="text" value="" class="form-control onlynumeric" maxlength="10" placeholder="Mobile No." required>
                    </div>                     
                    <div class="form-group">                        
                   ...
                   <div class="clearfix"></div>
                    <div class="form-group" >
                        <button type="submit" name="Submittt" style="margin-top:20px;" class="btn btn-default">Submit</button>
                    </div>  
                </form>
            </div>
        </div>
